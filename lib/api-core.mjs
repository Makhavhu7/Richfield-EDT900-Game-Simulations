...
async function publishedFile(context, settings) {
    ...
    try {
        body = await fs.promises.readFile(
            path.join(settings.publishedRoot, ...parts),
            "utf8"
        );
    } catch (error) {
        return null;
...
export function createApi({ catalog, kv, config = {} }) {
    ...
    async function handleHealth(context) {
        ...
        // /api/health?debug=1 also reports whether the pages and assets
        // are available to this function (see README, "hosting").
        if (
            String(context.query?.debug || "") === "1" &&
            settings.publishedRoot
        ) {
            payload.published = publishedReport(
                settings
            );
        }
